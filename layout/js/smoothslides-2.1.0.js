	...
	$.fn.extend({
		smoothSlides: function(options) {
			...
			if(options.matchImageSize == 'true') {
				...
				$('#'+uniqueId+' img').css('maxWidth',maxWidth);
			} else {
				$('#'+uniqueId+' img').css('width',maxWidth);
			}
			...
			function captionUpdate() {
				...
				if (!nextCaption) {
					...
					$('#'+uniqueId).find(".ss-caption").css('opacity','0.5').html(nextCaption);
				}
			}
			// Captions backward
			...
			if (options.pagination == 'true') {
				$(that).append('<div class="ss-paginate-wrap hidden-xs"><div class="ss-paginate"></div></div>');
			...
			$('#'+uniqueId).on('click', '.ss-paginate a', function(event) {
				...
				if (!nextCaption) {
					...
					$('#'+uniqueId).find(".ss-caption").css('opacity','.5').html(nextCaption);
				}
				event.preventDefault();
			});
